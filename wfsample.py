...
dwf.FDwfAnalogInTriggerLevelSet(hdwf, c_double(2.0)) # 0.5V
...
ax.set_title("Waveforms Ch1", size = 16)
ax.plot(numpy.fromiter(rgdSamples, dtype=numpy.float))
ax.set_xlim(0, 1000)
graph_name = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
fig.savefig("graphsg/%s" % graph_name)
print("save graph {}".format(graph_name))
